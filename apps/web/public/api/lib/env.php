...
function mt_load_env(?string $startDir = null): array {
    ...
    foreach (['MYSQL_HOST', 'MYSQL_USER', 'MYSQL_PASSWORD', 'MYSQL_DATABASE', 'MYSQL_PORT', 'DATABASE_URL', 'MANAGER_EMAIL', 'MANAGER_PASSWORD', 'MANAGER_NAME', 'AUTH_SECRET', 'SMTP_FROM', 'SMTP_HOST', 'HOSTINGER_EMAIL_MCP_TOKEN', 'HOSTINGER_MAIL_MAILBOX_ID', 'TURNSTILE_SITE_KEY', 'TURNSTILE_SECRET_KEY'] as $key) {
        $g = getenv($key);
        if ($g !== false && $g !== '') {
            $merged[$key] = $g;
        }
